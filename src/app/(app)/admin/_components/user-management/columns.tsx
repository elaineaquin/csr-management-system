...
export const columns: ColumnDef<UserWithRole>[] = [
	{
		...
		accessorKey: 'name',
		cell: ({ row }) => {
			return <UserInfoCard userId={row.original.id} userName={row.original.name} />;
		},
	},
	{
		accessorKey: 'email',
		...
		cell: ({ row }) => {
			...
			return (
				<span className="flex items-center gap-2">
					{user.email}{' '}
					{user.emailVerified ? <CheckCircleIcon className="w-4 h-4" /> : <XCircleIcon className="w-4 h-4" />}
				</span>
			);
		},
	},
	{
		...
		cell: ({ row }) => {
			const user = row.original;
			const roles = user.role?.split(',').map((role: string) => role.trim()) ?? [];
	...
	},
	{
		...
		cell: ({ row }) => {
			...
			const timeLeftText = banExpires && formatDistanceToNow(banExpires, { addSuffix: true });

			return (
				<HoverCard>
					<HoverCardTrigger>
						<Badge variant={user.banned ? 'destructive' : 'outline'}>{user.banned ? 'Yes' : 'No'}</Badge>
					</HoverCardTrigger>
