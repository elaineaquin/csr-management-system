...
import { GetAllRequestResult } from '@/server/volunteer';
import { ButtonGuard } from '@/components/button-guard';
import Link from 'next/link';
import { VolunteerApplicationForm } from '@/components/application-form';

export function PostRequest({ request }: { request: GetAllRequestResult[number] }) {
				...
				<div className="flex items-center justify-between">
					<div>
						...
						<CardDescription className="flex items-center gap-2 mt-1">
							Created by
							<UserInfoCard userId={request.createdById} userName={request.createdBy.name} />
						</CardDescription>
					</div>
			...
			<CardFooter className="bg-muted/50 flex justify-between items-center">
				<div className="flex items-center gap-4">
					...
					<div className="flex items-center gap-2 text-sm">
						...
						<span>
							Seeking {request.participantLimit} volunteer{request.participantLimit > 1 ? 's' : ''}
						</span>
					</div>
				</div>
				<div className="flex items-center gap-2">
					...
					{!isOwner && (
						<ButtonGuard name="volunteer" actions={['participate']}>
							<VolunteerApplicationForm requestId={request.id} requestStatus={request.status} />
						</ButtonGuard>
					)}
				</div>
			</CardFooter>
