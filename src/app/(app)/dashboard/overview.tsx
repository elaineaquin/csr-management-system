...
import { formatCurrency, formatDate, getProposalStatusColor, getVolunteerRequestStatusColor } from '@/lib/utils';
import { Project, VolunteerRequest } from '@prisma/client';
import { BarChart2, Calendar, Clock, FileText, Target, Users, Eye } from 'lucide-react';
			...
			<div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
					...
					<CardContent className="p-4">
						<div className="flex items-center justify-between">
							<div>
								...
								<p className="text-2xl font-bold text-blue-900 dark:text-white">{data.totalProjects}</p>
							</div>
				...
				<Card className="bg-gradient-to-r from-green-50 to-green-100 border-green-200 dark:from-green-900 dark:to-green-800 dark:border-green-700">
					<CardContent className="p-4">
						<div className="flex items-center justify-between">
							<div>
								<p className="text-sm font-medium text-green-600 dark:text-green-300">Available Budget</p>
				...
				<Card className="bg-gradient-to-r from-purple-50 to-purple-100 border-purple-200 dark:from-purple-900 dark:to-purple-800 dark:border-purple-700">
					<CardContent className="p-4">
						<div className="flex items-center justify-between">
							<div>
								<p className="text-sm font-medium text-purple-600 dark:text-purple-300">Volunteers</p>
								<p className="text-2xl font-bold text-purple-900 dark:text-white">{data.volunteersCount}</p>
							</div>
				...
				<Card className="bg-gradient-to-r from-orange-50 to-orange-100 border-orange-200 dark:from-orange-900 dark:to-orange-800 dark:border-orange-700">
					<CardContent className="p-4">
						<div className="flex items-center justify-between">
							<div>
								<p className="text-sm font-medium text-orange-600 dark:text-orange-300">Completed Projects</p>
			...
			<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
				<Card className="h-fit">
					<CardHeader>
						...
						<CardDescription>View and manage recent project proposals.</CardDescription>
					</CardHeader>
					<CardContent className="p-4">
				...
				<Card className="h-fit">
					<CardHeader>
						...
						<CardDescription>Analyze the budget distribution across projects.</CardDescription>
					</CardHeader>
					<CardContent className="p-4">
				...
				<Card className="h-fit">
					<CardHeader>
						...
						<CardDescription>Track volunteer participation and engagement.</CardDescription>
					</CardHeader>
					<CardContent className="p-4">
						<VolunteerRequests recentVolunteerRequests={data.recentVolunteerRequests} />
					</CardContent>
				</Card>
			</div>
		</div>
			...
			{recentVolunteerRequests.slice(0, 3).map((req) => (
				...
				>
					<div className="flex items-start justify-between mb-2">
						...
						<Badge className={`text-xs ${getVolunteerRequestStatusColor(req.status)}`}>{req.status}</Badge>
					</div>
			...
			{projects.slice(0, 3).map((project) => (
				...
				>
					<div className="flex items-start justify-between mb-2">
						...
						<Badge className={`text-xs ${getProposalStatusColor(project.status)}`}>{project.status}</Badge>
					</div>
					<div className="flex items-center justify-between text-xs text-muted-foreground">
						<div className="flex items-center gap-3">
							<span className="flex items-center gap-1">{formatCurrency(project.budget)}</span>
							<span className="flex items-center gap-1">
								<Calendar className="w-3 h-3" />
