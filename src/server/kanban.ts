...
export async function getKanbanBoard({ projectId }: { projectId: string }) {
	...
	return await prisma.kanbanBoard.create({
		data: {
			...
			members: { connect: participants?.volunteers.map((v) => ({ id: v.userId })) ?? [] },
			columns: {
				create: [
...
export async function getKanbanFormRequirements({ projectId }: { projectId: string }) {
	...
	const columnsWithPosition = board.columns.map((column) => ({
		...
		lastPosition: column.cards.length ? Math.max(...column.cards.map((c) => c.position)) : 0,
	}));
...
export async function updateTaskCardPosition({
	...
	try {
		await prisma.kanbanCard.update({ where: { id: taskId }, data: { columnId: columnId, position: position } });
	} catch (error) {
		console.error(error);
	}
}

export async function updateColumnPosition({ columnId, position }: { columnId: string; position: number }) {
	try {
		await prisma.kanbanColumn.update({ where: { id: columnId }, data: { position } });
	} catch (error) {
		console.error(error);
	}
}

export async function updateColumnColor({ columnId, color }: { columnId: string; color: KanbanColorType }) {
	try {
		await prisma.kanbanColumn.update({ where: { id: columnId }, data: { color } });
	} catch (error) {
		console.error(error);
	}
}
...
export async function renameColumn({ columnId, title }: { columnId: string; title: string }) {
	try {
		await prisma.kanbanColumn.update({ where: { id: columnId }, data: { title } });
	} catch (error) {
		console.error(error);
	}
}
...
export async function deleteTask({ taskId }: { taskId: string }) {
	try {
		const existing = await prisma.kanbanCard.findUnique({ where: { id: taskId } });
		if (!existing) {
