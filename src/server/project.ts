...
export async function createRequestParticipants(
  data: Prisma.VolunteerRequestCreateInput
) {
  try {
    const newRequest = await prisma.volunteerRequest.create({
      data,
...
export async function createProjectHistory(
  data: Prisma.ProjectHistoryCreateInput
) {
...
export async function canAccessKanban({
  ...
  const allVolunteerUserIds = project.volunteerRequests.flatMap((vr) =>
    vr.volunteers.map((v) => v.userId)
  );
...
export async function getProjectParticipants({
  ...
  // Filter users who have both 'u2' and 'u4' in their roles
  const participants = users.filter((user) => {
    const roles = (user?.role ?? "").split(","); // Split the comma-separated roles
    return roles?.includes("u2") && roles?.includes("u4");
  });

  // Include the creator in the participants list
  const finalParticipants = [
    creator,
    ...participants.filter((user) => user.id !== creator?.id),
  ];
