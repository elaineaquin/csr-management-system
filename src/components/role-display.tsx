...
import { ShieldCheck, User, Users, Briefcase, DollarSign, Handshake, Heart, HelpCircle } from 'lucide-react';
...
export const roleMap: Record<RoleKey, { label: string; icon: React.ReactNode; description?: string }> = {
	admin: {
		label: 'Admin',
		icon: <ShieldCheck className="w-4 h-4 text-blue-600 dark:text-blue-400 mr-2" />,
	...
	u1: {
		...
		icon: <Users className="w-4 h-4 text-emerald-600 dark:text-emerald-400 mr-2" />,
		description: 'Liaisons for CSR-related communication',
	},
	u2: {
		label: 'Leadership & Approvers',
		icon: <Briefcase className="w-4 h-4 text-purple-600 dark:text-purple-400 mr-2" />,
		description: 'Directors, CEOs, and decision makers',
	},
	u3: {
		label: 'Finance & Procurement',
		icon: <DollarSign className="w-4 h-4 text-green-600 dark:text-green-400 mr-2" />,
		description: 'Handles financial and purchasing operations',
	},
	u4: {
		label: 'Partner Organizations',
		icon: <Handshake className="w-4 h-4 text-yellow-600 dark:text-yellow-400 mr-2" />,
