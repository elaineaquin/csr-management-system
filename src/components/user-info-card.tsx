...
export function UserInfoCard({ userId, userName }: { userId: string; userName?: string }) {
			...
			<HoverCardContent align="start" className="w-80 p-0 overflow-hidden">
				{isLoading ? (
					...
					<div className="flex flex-col">
						{/* Banner */}
						<div className={cn('h-20', data?.id ? getBannerColor(data.id) : 'bg-gray-500')} />
						...
						<div className="px-4 pb-4 pt-6 relative">
							...
							<div className="absolute -top-10 left-4 rounded-full">
								<Avatar className="w-16 h-16">
									...
									<AvatarFallback className="text-lg">{data?.name?.[0]?.toUpperCase() ?? '?'}</AvatarFallback>
								</Avatar>
							</div>
							...
							<div className="space-y-2">
								<div>
									<div className="flex items-center gap-2">
										...
										{data?.role === 'admin' && <Shield className="h-4 w-4 text-blue-500" />}
									</div>
								...
								{/* Additional info */}
								<div className="space-y-2">
									<h4 className="text-xs font-semibold uppercase text-muted-foreground">About Me</h4>
								...
								{data?.createdAt && (
									<div className="space-y-1">
										<h4 className="text-xs font-semibold uppercase text-muted-foreground">Member Since</h4>
										<p className="text-xs">
