...
import { HoverCard, HoverCardContent, HoverCardTrigger } from '@/components/ui/hover-card'; // Adjust path based on your shadcn setup
		...
		>
			<CardHeader className="flex flex-row items-center justify-between border-b border-white/10 bg-transparent px-2 py-0">
				<div className="flex items-center gap-1">
					...
					<Badge variant="outline" className="bg-white/20 border-white/30 backdrop-blur-sm">
						Task
					</Badge>
				</div>
				<Button variant="ghost" size="icon" onClick={() => setOpenDeleteDialog(true)}>
			...
			<CardContent className="p-3 flex flex-col gap-2 text-left bg-transparent">
				<div className="font-semibold text-sm hover:underline cursor-pointer" onClick={() => setOpenTaskDrawer(true)}>
					{task.title}
				</div>

				{task.description && <div className="text-sm whitespace-pre-wrap line-clamp-3">{task.description}</div>}
					...
					{task.assignees?.length > 0 && (
						<HoverCard>
							<HoverCardTrigger asChild>
								<span className="ml-auto underline cursor-pointer">{task.assignees.length} assignee(s)</span>
							</HoverCardTrigger>
							<HoverCardContent className="w-64 p-3 shadow-lg backdrop-blur-sm bg-white/10 border dark:border-white/10 rounded-md dark:text-white">
								<div className="mb-2 border-b pb-1 text-sm font-semibold">Assigned to</div>
								<div className="flex flex-col gap-2">
									{task.assignees.map((assignee) => (
										<UserInfoCard key={assignee.user.id} userId={assignee.user.id} userName={assignee.user.name} />
									))}
								</div>
							</HoverCardContent>
						</HoverCard>
			...
			<Drawer open={openTaskDrawer} onOpenChange={setOpenTaskDrawer} direction="right">
				<DrawerContent>
					<DrawerHeader className="border-b-2 relative">
						<div className="flex items-center justify-between">
							...
							<div className="flex gap-2">
								<Button variant="ghost" size="icon" onClick={() => setOpenEditTitle(true)} aria-label="Edit task title">
									<PencilIcon className="h-5 w-5" />
								</Button>
								<Button variant="ghost" size="icon" onClick={() => setOpenDeleteDialog(true)} aria-label="Delete task">
