						...
						<DropdownMenuContent className="w-56">
							<DropdownMenuGroup>
								...
								<DropdownMenuSub>
									...
									<DropdownMenuSubContent>
										{(['Red', 'Green', 'Blue', 'Violet', 'Orange', 'Yellow', 'Gray'] as KanbanColorType[]).map(
											(color) => (
												<DropdownMenuItem
													key={color}
													onClick={async () => {
												...
												>
													<CircleIcon className={`w-3 h-3 mr-2 text-${color.toLowerCase()}-500`} fill="currentColor" />
													{color}
												</DropdownMenuItem>
											),
										)}
			...
			<ScrollArea className="flex-1 overflow-hidden">
				<CardContent className="flex flex-col gap-2 p-2 px-4 h-full overflow-y-auto">
					<SortableContext items={tasksIds}>
						{tasks.map((task) => (
							<TaskCard key={task.id} task={task} onDelete={deleteTask} renameTask={renameTask} />
						))}
					</SortableContext>
				</CardContent>
			</ScrollArea>
			<Button className="w-full flex justify-between" variant={'ghost'} onClick={() => setAddTaskDialogOpen(true)}>
				Add Task
				<PlusCircleIcon />
			</Button>
				...
				<AlertDialogContent>
					...
					<AlertDialogFooter>
						...
						<Button
							onClick={() => {
								if (renameValue.trim()) {
									renameColumn?.({ columnId: column.id, title: renameValue.trim() });
			...
			<AlertDialog open={openDeleteDialog} onOpenChange={setOpenDeleteDialog}>
				<AlertDialogContent>
					<AlertDialogHeader>
						...
						<AlertDialogDescription>
							This will permanently delete the column <strong>{column.title}</strong> and all its tasks.
						</AlertDialogDescription>
					</AlertDialogHeader>
					<AlertDialogFooter>
...
export function BoardContainer({ children }: { children: React.ReactNode }) {
	...
	return (
		...
		>
			<div className="flex gap-4 items-center flex-row justify-start">{children}</div>
			<ScrollBar orientation="horizontal" />
		</ScrollArea>
	);
}

export function getColumnColorClass(color: KanbanColorType | undefined | null): string {
	if (!color) {
