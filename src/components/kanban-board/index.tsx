...
export function KanbanBoard({ board }: { board: KanbanBoardType }) {
  ...
  const [columns, setColumns] = useState<KanbanBoardColumnType[]>(
    () => board.columns
  );
  const [tasks, setTasks] = useState<KanbanBoardCardType[]>(() =>
  ...
  const [activeColumn, setActiveColumn] =
    useState<KanbanBoardColumnType | null>(null);
  const [activeTask, setActiveTask] = useState<KanbanBoardCardType | null>(
    null
  );
  ...
  const handleOnRenameTask = async ({
    taskId,
    title,
  }: {
    taskId: string;
  ...
  const handleAddTask = async ({ task }: { task: KanbanBoardCardType }) => {
    setTasks((prev) => {
      const tasksInTargetColumn = prev.filter(
  ...
  function onDragEnd(event: DragEndEvent) {
    ...
    if (isActiveATask) {
      setTasks((prevTasks) => {
        ...
        if (!activeTask || (!overTask && overData?.type !== "Column"))
          return prevTasks;
        ...
        if (overData && overData.type === "Column") {
          ...
          const tasksInTarget = updatedTasks.filter(
          ...
          insertIndex =
            activeTask.columnId === targetColumnId && activeIndex < overIndex
              ? overIndex - 1
              : overIndex;
        }
        ...
        const allColumnsToUpdate = new Set([
          activeTask.columnId,
          targetColumnId,
        ]);
        ...
        const untouched = updatedTasks.filter(
          (t) => !allColumnsToUpdate.has(t.columnId)
        );
